...
def follow_by_link(
    ...
    """
    Sends the "follow_by_link" task and returns when the task is completed.

    :param token: Nomix token
    :param device_uuids: devices to which the task will be sent
    :param link: link to the account to follow
    ...
    if not response.ok:
        logger.error(f"Failed to send \"follow_by_link\": {response.text}")
        return False

    task_uuid = get_task_uuid_from_server_response(response.text)
    logger.info(f"Task \"follow_by_link\" sent: {task_uuid}")
    ...
    while True:
        ...
        if not response.ok:
            logger.error(f"Failed to check \"follow_by_link\" status: {response.text}")
            return False
        ...
        if task_status == TaskStatus.SUCCESS:
            logger.info(f"Task \"follow_by_link\" ({task_uuid}) completed successfully")
            return True
        elif task_status == TaskStatus.ERROR:
            logger.error(f"Task \"follow_by_link\" ({task_uuid} completed with error")
            return False
